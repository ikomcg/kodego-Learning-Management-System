<?php   
    ...
    if(isset($_GET['ID']) && isset($_GET['subID'])){
        ...
        // view answer
        $sqlsubmission = "SELECT * FROM submission WHERE Assignment_ID = '$assID' AND Submission_ID = '$subID '";
        $resultSubmission = $mysqli->query($sqlsubmission);
        $result_sub = $resultSubmission->fetch_assoc(); 

        $next = "SELECT * FROM submission WHERE Assignment_ID = $assID AND Submission_ID > '$subID' ORDER BY Submission_ID ASC";
        $next_row = $mysqli->query($next);

        if($row = mysqli_fetch_array($next_row)){

              $next_SubmissionID = $row['Submission_ID'];
        }
        $previous = "SELECT * FROM submission WHERE Assignment_ID = $assID AND Submission_ID<'$subID' ORDER BY Submission_ID ASC";
        $result_prev = $mysqli->query($previous);

        if($prev_row = mysqli_fetch_array($result_prev)){

            $prev_SubmissionID = $prev_row['Submission_ID'];  
        }
            ...
            <div class="wrap d-flex ">
                        ...
                        <ul class="d-flex">
                            ...
                            <?php 
                                if($user_type == 'Teacher' ){
                                    echo '
                                    <li ><a href="student_grade.php?course='.$courseName.'&ID='.$courseID.'&courseCode='.$resultInfo['Course_Code'].'">Grade</a></li>
                                    ';
                                }
                            ?>
                        </ul>
                ...
                <div class="right-modul">
                    <div class="top-right">
                        ...
                        <ul>
                        <li>
                                Student: <?php echo $result_sub['Student_Name'] ?>
                            </li>
                            <li>
                            ...
                            <!-- button next view  href="" -->

                            <div class="d-flex justify-content-center align-items-center g-5">
                                <a style="background-color: transparent; color:#14599d; font-size:24px; padding:0; margin: 0 40px;" href=" grade.php?course=<?php echo $courseName ?>&ID=<?php echo $courseID?>&assID=<?php echo $assID ?>&subID=<?php echo $prev_SubmissionID ?>"> <i class="bi bi-arrow-left-circle"></i></a>
                                <a style="background-color: transparent; color:#14599d; font-size:24px; padding:0; margin:0 40px;" href=" grade.php?course=<?php echo $courseName ?>&ID=<?php echo $courseID?>&assID=<?php echo $assID ?>&subID=<?php echo $next_SubmissionID ?>"><i class="bi bi-arrow-right-circle"></i></a>
                            </div>
                        </ul>
                    </div>
                    <div class="mid-right">
                        ...
                        <ul>
                            <li>
                                <form action="data/grade.php" method="POST">
                                    <div class="m-2">
                                        <label for="">Score: </label>
                                        <input value="<?php echo $result_sub['Score']?>" type="text" name="score" style="width:40%; text-align: center; border: none;  outline: none;"  />
                                        <input type="hidden" name="Sub_ID" value="<?php echo $subID;?>">
                                    </div>
                                    <button type="submit" name="Post_Grade" style="width: fit-content; color:white; font-size:16px; padding:2px 8px;">Post Grade</button> 
                                </form>
                            </li>
                        </ul>
                    </div>
                    <div class="mid-right">
                        ...
                        <ul>
                            <li>
                                <a href="submission/<?php echo $result_sub['Assignment_Ans']?>" target="_blank" type="application/pdf">View answer</a>
                            </li>
                        </ul>
                    </div>
                    <div class="bot-right">
